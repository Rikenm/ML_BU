...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    empty_matrix = np.zeros((scores.shape))

 

    for i in range(N):
      empty_matrix[i][y[i]]=1
 


    dscore = (P - empty_matrix)
    ...
    # [PLEASE IMPLEMENT]
    db1 = np.mean(dz1,axis=0)#None  # [PLEASE IMPLEMENT]
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for epoch in xrange(num_epochs):
        ...
        for it in xrange(iterations_per_epoch):
            X_batch = None #X[it*batch_size:(it+1)*batch_size]
            y_batch = None #y[it*batch_size:(it+1)*batch_size]
